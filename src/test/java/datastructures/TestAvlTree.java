...
public class TestAvlTree extends BaseTest {
    ...
    public void testValidAVLTreeBasic() {
        BST tree = new BST(new int[]{2, 1, 3}); // Creates a Binary Search Tree with the given values in an inorder traversal order.
    ...
    public void testInvalidAVLTreeBasic() {
        BST tree = new BST(new int[]{1, 2, 3}); // Creates a Binary Search Tree with the given values in an inorder traversal order.
     ...
     * Instructions:
     *
     * We will be running all the tests in this file against 11 different implementations of isValidAVL() method (These implementations are not accessible to you.)
     ...
     * Recall from class: a black box test is when you do not know the implementation, but you know the expected behavior.
     *
     *
     *
     * You get points for correctly predicting whether an isValidAVL method implementation is buggy or correct.
     * There are 11 implementations (one correct, 10 buggy), each worth 1 point. If you correctly identify all 11, you get 11 points.
